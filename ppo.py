...
def parse_args():
    ...
    parser.add_argument('--anneal-lr', type=lambda x:str2bool(x), default=True, nargs='?', const=True,
                        help="Toggle learning rate annealing for policy and value networks")
    parser.add_argument('--gae', type=lambda x:str2bool(x), default=True, nargs='?', const=True,
    ...
    parser.add_argument('--norm-adv', type=lambda x:str2bool(x), default=True, nargs='?', const=True,
                        help="Toggles advantages normalization")
    parser.add_argument('--clip-coef', type=float, default=0.2,
                        help="the surrogate clipping coefficient")
    parser.add_argument('--clip-vloss', type=lambda x:str2bool(x), default=True, nargs='?', const=True,
    ...
    parser.add_argument('--max-grad-norm', type=float, default=0.5,
                        help= "the maximum norm of the gradient")
    parser.add_argument('--target-kl', type=float, default=None, # 0.015
                        help= "the target kl value for the KL divergence threshold")
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        clip_fracs = []
        for epoch in range(0, args.batch_size, args.minibatch_size):
        ...
        # TRY NOT TO MODIFY
        writer.add_scalar("charts/learning_rate", optimizer.param_groups[0]['lr'], global_step=global_step)
        writer.add_scalar("losses/value_loss", v_loss.item(), global_step=global_step)
        writer.add_scalar("losses/policy_loss", pg_loss.item(), global_step=global_step)
        writer.add_scalar("losses/entropy", entropy_loss.item(), global_step=global_step)
        # writer.add_scalar("losses/old_approx_kl", old_approx_kl.item(), global_step=global_step)
        writer.add_scalar("losses/approx_kl", approx_kl.item(), global_step=global_step)
        writer.add_scalar("losses/clipfrac", np.mean(clip_fracs), global_step=global_step)
        writer.add_scalar("losses/explained_variance", explained_var, global_step=global_step)
        # if args.kle_stop or args.kle_rollback:
        #     writer.add_scalar("debug/pg_stop_iter", i_epoch_pi, global_step=global_step)
        print("SPS:", int(global_step / (time.time() - start_time)))
        writer.add_scalar("charts/SPS", int(global_step / (time.time() - start_time)), global_step=global_step)
